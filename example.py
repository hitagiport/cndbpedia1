...
FLOOR_IMGS = ['Ear_Of_Rice', 'Herb', 'Snail', 'Spiral_Shell', 'Turtle', 'Anchor', 'Pile_Of_Poo', 'Sailboat',
              'Speedboat']
...
class Tower(object):
    ...
    def create_towers_and_caps(self):
        ...
        self.lower_cap = Rect(self.x, self.env.ground_height + height - self.env.tower_cap, self.env.tower_width,
                              self.env.tower_cap)
        self.upper_tower = Rect(self.x + 6, height + self.env.gap, self.env.tower_width - 12,
                                self.env.playfield.h - height + self.env.gap)
...
class Game(object):
    ...
    def create_game_objects(self):
        ...
        for t in range(10):
            ...
            self.bubbles.append(
                Bubble(random.randint(0, self.env.playfield.w), random.randint(0, self.env.playfield.h), d, d,
                       random.randint(self.env.float_min, self.env.float_max)))
        self.floor_sprites = []
        for t in range(1):
    ...
    def move_player(self, dt):
        if (self.env.state == GAME_DEAD):
            return
        elif ((self.env.state == GAME_READY) and (
            self.player.bounds.y < (self.env.playfield.h / 2)) or self.player.jumped):
    ...
    def move_bubbles(self, dt):
        ...
        for bubble in self.bubbles:
            if (bubble.bounds.bottom() > self.env.playfield.top()) or (
                bubble.bounds.left() < self.env.playfield.left()):
        ...
        if (self.env.state == GAME_READY) or (self.env.state == GAME_PLAY):
            ...
            for sprite in self.floor_sprites:
                ...
                if sprite.bounds.right() < self.env.playfield.left():
                    ...
                    sprite.bounds.x = random.randint(self.env.playfield.right(),
                                                     self.env.playfield.right() + self.env.playfield.w)
    ...
    def move_background_sprites(self, dt):
        if (self.env.state == GAME_READY) or (self.env.state == GAME_PLAY):
            for sprite in self.background_sprites:
                ...
                sprite.bounds.x -= move
                if (sprite.bounds.right() < self.env.playfield.left()) or (
                    sprite.bounds.left() > self.env.playfield.right()):
                    sprite.set_random_image()
                    sprite.set_random_bounds()
